// Modify based on https://github.com/nuxt/image/blob/main/src/runtime/providers/sanity.ts
// to optionally load image from base URL
...
const operationsGenerator = createOperationsGenerator({
    ...
    joinWith: '&',
    formatter: (key: string, value: string) => String(value) === 'true' ? key : `${key}=${value}`,
})
...
export const getImage: ProviderGetImage = (src, { modifiers = {}, projectId, dataset = 'production' } = {}) => {
    const baseUrl = useRuntimeConfig().public.sanityImageBaseUrl || 'https://cdn.sanity.io/images'
